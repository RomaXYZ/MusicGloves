...
//#define TFT_CS                  (*((volatile uint32_t *)0x40004008)) // PA1
...
void static commonInit(const uint8_t *cmdList) {
  ...
  GPIO_PORTA_DEN_R |= 0xC8;             // enable digital I/O on PA3,6,7
	//GPIO_PORTA_DIR_R |= 0xC2;             // make PA1,6,7 out
  //GPIO_PORTA_AFSEL_R &= ~0xC2;          // disable alt funct on PA1,6,7
  //GPIO_PORTA_DEN_R |= 0xC2;             // enable digital I/O on PA1,6,7
		
                                        // configure PA3,6,7 as GPIO
  GPIO_PORTA_PCTL_R = (GPIO_PORTA_PCTL_R&0x00FF0FFF)+0x00000000;
	//GPIO_PORTA_PCTL_R = (GPIO_PORTA_PCTL_R&0x00FFFF0F)+0x00000000;	// configure PA1,6,7 as GPIO
	GPIO_PORTA_AMSEL_R &= ~0xC8;          // disable analog functionality on PA3,6,7
	//GPIO_PORTA_AMSEL_R &= ~0xC2;          // disable analog functionality on PA1,6,7	
...
/**************ST7735_XYplotInit***************
 ...
 Inputs:  title  ASCII string to label the plot, null-termination
          minX   smallest X data value allowed, resolution= 1/32
          maxX   largest X data value allowed, resolution= 1/32
          minY   smallest Y data value allowed, resolution= 1/32
          maxY   largest Y data value allowed, resolution= 1/32
 Outputs: none
 assumes minX < maxX, and miny < maxY
*/
int32_t low_x, high_x, low_y, high_y;
...
//	ST7735_FillRect(0, 32, 128, 128, ST7735_Color565(255, 255, 255));
	
	// Specify the X and Y axes...
	low_x = minX; // "low_x maps to (0, Y)
	high_x = maxX; // "high_x maps to (MAX, Y)
	low_y = minY; // "low_y maps to (X, 0)
	high_y = maxY; // "high_y maps to (X, MAX)
...
/**************ST7735_XYplot***************
 ...
 Inputs:  num    number of data points in the two arrays
          bufX   array of 32-bit fixed-point data, resolution= 1/32
          bufY   array of 32-bit fixed-point data, resolution= 1/32
 Outputs: none
...
void ST7735_XYplot(uint32_t num, int32_t bufX[], int32_t bufY[]){
	...
	// Determine x and y conversion factor...
	int32_t x_conversion, y_conversion, x_shift, y_shift;
	x_conversion = (128 * 100000) / (high_x - low_x);
	y_conversion = (160 * 100000) / (high_y - low_y);
	x_shift = 0;
	y_shift = 0;
	
	// Is xMin in negative territory?
	if(low_x < 0){
		// Convert the low_x to it's equivalent in LCD...
		x_shift = (low_x * x_conversion) / 100000;
		// Denegative it...
		x_shift = x_shift * (-1);
	}
	// Is yMin in negative territory?
	if(low_y < 0){
		// Convert the low_y to it's equivalent in LCD...
		y_shift = (low_y * y_conversion) / 100000;
		// Denegative it...
		y_shift = y_shift * (-1);
	}
	...
	for(uint32_t index = 0; index < num; index ++){
		...
		if((x < low_x) || (x > high_x) || (y < low_y) || (y > high_y)){
			...
			// Point is good... translate!
			uint32_t point_x, point_y;
			
			// Translate the x coordinate of the point...
			point_x = ((bufX[index] * x_conversion) / 100000) + x_shift;
			point_y = 160 - (((bufY[index] * y_conversion) / 100000) + y_shift);
			// Print the pixel onto the LCD...
			
			ST7735_DrawPixel(point_x, point_y, ST7735_Color565(255, 0, 0));
...
/**************get_exponential***************
 10^x
 Inputs:  X = number of places
          
 Outputs: 32-bit signed representation of 10^x
*/
int32_t get_exponential(int16_t power){
	int32_t result = 1;
	
	if(power == 0){
		return(result); // 10^0 equals 1
	}
...
/**************Extract_signed***************
 Extracts the digit from which ever place starting from the right going left...
 Inputs:  number A 32-bit signed integer 
          place  Which place you want to extract from
          
 Outputs: 16-bit unsigned representation of that extracted number

 Example: Extract_signed(12345, 0) = 5
					Extract_signed(12345, 1) = 4
					Extract_signed(12345, 2) = 3
					Extract_signed(12345, 3) = 4
					Extract_signed(12345, 4) = 1

*/
int16_t Extract_signed (int32_t number, int16_t place){
	number = number / get_exponential(place); // Shrinks number by place number places
	
	number = number % 10; // Extract the least significant number
...
/**************Extract_unsigned***************
 Extracts the digit from which ever place starting from the right going left...
 Inputs:  number A unsigned 16-bit integer 
          place  Which place you want to extract from
...
/**************IntegerToChar**************
 Turns an integer value between 0-9 and turn it into its ASCII char equivalent
 Inputs:  An 16-bit signed integer.
          
 Outputs: A character representation in ASCII of that integer.
...
void ST7735_sDecOut3(int32_t n){
	...
	// Draw!
		...
		char *String_Builder;
		// Allocate a string with 6 characters in it...
		// Leave one at the end to store null-termination 
		// and one up front for the negation marker...
		String_Builder = (char*)malloc(8*sizeof(char));
		
		int32_t input = n; 
		// Is it a negative number or positive number?
		if(n < 0){
				// Negative
				String_Builder[0] = '-';
				input = input * -1; // Turn working number positive
		}else{
			// Positive
			String_Builder[0] = ' ';
		}
		
		// Determine the value of the 10 thousand's place...
		int16_t temporal = Extract_signed(input, 4); // Temporal is spanish for temporary
		// Check for empty zero's
		if(temporal == 0){
			String_Builder[1] = ' ';
		}else{
			String_Builder[1] = IntegerToChar(temporal);
		}
		
		// Determine the value of the thousand's place...
		temporal = Extract_signed(input,3);
		String_Builder[2] = IntegerToChar(temporal);
		
		// Set the period...
//		String_Builder[3] = '.';
		String_Builder[3] = ':';
		
		// Determine the value of the hundred's place...
		temporal = Extract_signed(input, 2);
		String_Builder[4] = IntegerToChar(temporal);
		
		// Determine the value of the ten's place....
		temporal = Extract_signed(input, 1);
		String_Builder[5] = IntegerToChar(temporal);
		
		// Determine the value of the one's place...
		temporal = Extract_signed(input, 0);
		String_Builder[6] = IntegerToChar(temporal);
		
		
		// Add the null-terminator to designate this as a string...
		String_Builder[7] = NULL;
		
		
		// Send the string to the LCD
		ST7735_OutString(String_Builder);
...
/**************ST7735_uBinOut5***************
 unsigned 32-bit binary fixed-point with a resolution of 1/32. 
 The full-scale range is from 0 to 999.99. 
...
void ST7735_uBinOut5(uint32_t n){
	...
	if(n > 31999){
		...
		// No errors detected! Convert!
		
	// Blow it up...
		uint32_t blown_up = n * 100;
		
		char *String_Builder;
		// Allocate a string with 6 characters in it...
		// Leave one at the end to store null-termination 
		String_Builder = (char*)malloc(7*sizeof(char));
		
		uint32_t temporal = Extract_unsigned((blown_up / 32), 4);
		
		// Determine the ten thousandth's place...
		if(temporal == 0){
			// There exists a zero! Block it out!
			String_Builder[0] = ' ';
			
		}else{ // Record the number
			String_Builder[0] = IntegerToChar(temporal);
		}
		
		temporal = Extract_unsigned((blown_up / 32), 3);
		// Determine the thousandth's place...
		if((temporal == 0) && (String_Builder[0] == ' ')){
			String_Builder[1] = ' ';
		}else{
			String_Builder[1] = IntegerToChar(temporal);
		}
		
		temporal = Extract_unsigned((blown_up / 32), 2);
		// Determine the hundredth's place (0's cannot be skipped now
		String_Builder[2] = IntegerToChar(temporal);
		...
		temporal = Extract_unsigned((blown_up / 32), 1);
		// Determine the ten's place...
		String_Builder[4] = IntegerToChar(temporal);
		
		temporal = Extract_unsigned((blown_up / 32), 0);
		// Determine the ones's place...
		String_Builder[5] = IntegerToChar(temporal);
		
		// Insert null-terminator...
		String_Builder[6] = NULL;
